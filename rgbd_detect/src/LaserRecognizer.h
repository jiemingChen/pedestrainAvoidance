...
//    typedef void(LaserRecognizer::*pfunc)(void);
        ...
        std::vector<float> feature5Vector;
//        std::vector<std::vector<float>> peoplePosition;
        std::vector<std::pair<float,float>> candidatePosition;
        std::vector<std::pair<float,float>> peoplePosition;
...
public:
        ...
        std::vector<std::vector<SamplePoint>> segment(const sensor_msgs::LaserScanConstPtr&);
        void test();
        void dTreeTest();
