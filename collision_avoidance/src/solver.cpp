...
Solver::Solver():N_(10), x_(env_, 3), u_(env_, 2), slack_(env_, N_){
    Q={1, 1, 0};
    P={1, 1, 0};
    R={0.09, 0.8};
...
//                                     pow(xIter[1][j] - obstacles_saw.first[k][1] - obstacles_saw.second[k][1] * dt_people * j, 2)) / 2;
                    expr = (xIter[0][j] - obstacles_saw.first[k][0])*(x_[0][j] - obstacles_saw.first[k][0]) +
                           (xIter[1][j] - obstacles_saw.first[k][1])*(x_[1][j] - obstacles_saw.first[k][1]) +
                           slack[j * obstacles_size + k] - (pow(safe_dist_, 2) + pow(xIter[0][j]-obstacles_saw.first[k][0], 2) + pow(xIter[1][j] - obstacles_saw.first[k][1], 2)) / 2;
...
//                    cout<<cplex.getValue(slack[i])<<" ";
                    if(cplex.getValue(slack[i]) > 0.01){
                        cout <<"danger"<<endl;
                        violate_cnt ++;
                        if(violate_cnt>2){
                            flag = true;
...
//                cout<<"\n"<<endl;
            ...
            if(cnt>9){
                ...
                for(auto& ob: obstacles_saw.first){
                ...
                float u_steer = abs(safe_dist_-temp_d)*dir*0.7;
                if(u_steer>M_PI/4){
                    u_steer = M_PI/4;
                }
